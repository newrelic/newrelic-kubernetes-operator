...
var _ = Describe("ApmCondition reconciliation", func() {
	...
	var (
		...
		condition      *nrv1.AlertsAPMCondition
	...
	BeforeEach(func() {
		...
		condition = &nrv1.AlertsAPMCondition{
			...
			Spec: nrv1.AlertsAPMConditionSpec{
				nrv1.AlertsGenericConditionSpec{
					APMTerms: []nrv1.AlertConditionTerm{
						{
							Duration:     "30",
							Operator:     "above",
							Priority:     "critical",
			...
			Status: nrv1.AlertsAPMConditionStatus{
				AppliedSpec: &nrv1.AlertsAPMConditionSpec{},
				ConditionID: 0,
			},
		}
		namespacedName = types.NamespacedName{
	...
	Context("When starting with no conditions", func() {
		...
		Context("and given a AlertsAPMCondition that exists in New Relic", func() {
			JustBeforeEach(func() {
				condition = &nrv1.AlertsAPMCondition{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-condition",
